    ...
    <div class="col-md-12">
      <div class="box">
        <div class="box-header">
          <h3 class="box-title" id="judulsatuan">@lang("admin::event.detailevent_edit")</h3>
        </div>
        <!-- /.box-header -->
        <div class="box-body">
          <div class="container-fluid add-product">
          <form id="eventform" enctype="multipart/form-data" action="{{ route('detailevent-update',['id' => $detailevent->id]) }}" method="post" data-toggle="validator">
              {{csrf_field()}}
              ...
              <div class="row">
                ...
                <select name="event_id" id="event_id" class="form-control">
                  @foreach($events as $event)
                    <option value="{{$event->id}}" id="{{$event->id}}" {{$event->id==$detailevent->event_id?'selected':''}}>{{$event->name}} || {{$event->title}}</option>
                  @endforeach
                  </select>
              ...
              <div class="row">
                ...
                <div class="row">
                  <div class="col-md-9">
                  <input id="slug" type="text" class="form-control" name="slug" value="{{ $detailevent->slug }}">
                  </div>
                  <div class="col-md-3">
              ...
              <div class="row">
                ...
                <textarea id="note" cols="30" rows="2" class="form-control" name="note">{{ $detailevent->note }}</textarea>
              ...
              <div class="row">
                <div class="form-group col-md-12">
                  <input id="submit" type="submit" class="form-control btn btn-primary prod-submit" value="@lang('admin::event.detailevent_edit')">
                </div>
              </div>
          </form>
          </div>
...
var uploadedDocumentMap = {}
  Dropzone.options.imageDropzone = {
    ...
    init: function () {
      ...
      @if(isset($detailevent) && $detailevent->attachments)
        var files =
          {!! json_encode($detailevent->attachments) !!}
